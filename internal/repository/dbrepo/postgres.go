...
// GetUserByID rreturn user by id
...
// UpdateUser udapte user in database
func (m *postgresDBRepo) UpdateUser(u models.User) error {
	ctx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
	defer cancel()

	query := `update users set firs_name=$1, last_name=$2, email=$3, access_level=$4, updated_at=$5`
...
// UpdateReservation udapte reservation in database
...
// DeleteReservation delete a  reservation by ID in database
...
// InsertBlockForRoom inserts a roomrestriction
func (m *postgresDBRepo) InsertBlockForRoom(id int, startDate time.Time) error {
	...
	query := ` 
		...
		`
	_, err := m.DB.ExecContext(ctx, query, startDate, startDate.AddDate(0, 0, 1), id, 3, time.Now(), time.Now())
	if err != nil {
		return err
	}
	return nil
